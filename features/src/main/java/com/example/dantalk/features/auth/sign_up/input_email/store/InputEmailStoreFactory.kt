...
class InputEmailStoreFactory(
    ...
    fun create(): InputEmailStore =
        object : InputEmailStore, Store<Intent, State, Label> by factory.create<Intent, Nothing, Msg, State, Label>(
            ...
            executorFactory = coroutineExecutorFactory {
                ...
                onIntent<Intent.NavigateNext> {
                    ...
                    if(state().validation == InputEmailValidation.Valid) {
                        val userData = UserData(email = state().email, username = state().username)
    ...
    private suspend fun checkFieldsExists(email: String, username: String): InputEmailValidation =
        withContext(Dispatchers.IO) {
            when {
                userRepository.isValueExists("email", email) -> InputEmailValidation.EmailAlreadyExist
                userRepository.isValueExists("username", username) -> InputEmailValidation.UsernameAlreadyExist
                else -> InputEmailValidation.Valid
            }
        }
}